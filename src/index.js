...
// ─── REQUEST TIMEOUT ─────────────────────────────────────────────────────────
app.use((req, res, next) => {
  const timeoutMs = isUploadRoute(req) ? 180000 : 25000;
...
const pollLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: isProd ? 20 : 100000,
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: (req) =>
    req.headers.authorization?.split(' ')[1] || req.ip,
  message: {
    success: false,
