...
// ── NEW: Expo broadcast ───────────────────────────────────────────────────────
import { broadcastPushNotification } from '../services/expoPushService.js';
...
// Sends Expo push notification to all riders, customers, or both
...
export async function sendBroadcastNotification(req, res, next) {
  try {
    ...
    if (!title || typeof title !== 'string' || !title.trim()) {
      return err(res, 'title is required.', 400);
    }
    if (!body || typeof body !== 'string' || !body.trim()) {
      return err(res, 'body is required.', 400);
    }
