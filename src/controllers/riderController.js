...
try {
// Guard: req.body can be undefined if the body parser was skipped (e.g.
// the route was mistakenly classified as an upload route in index.js).
// Destructuring undefined throws “Cannot destructure property ‘dob’…”
// so we default to an empty object as a safety net.
const body = req.body ?? {};
...
if (!/^\d{4}-\d{2}-\d{2}$/.test(dob)) {
  return err(res, 'Invalid date format. Use YYYY-MM-DD (e.g. 1995-06-15).', 400);
}

const d = new Date(dob);
if (isNaN(d.getTime())) {
  return err(res, 'Invalid date of birth.', 400);
}

// Age check: must be 18–85
const [y, m, day] = dob.split('-').map(Number);
const today = new Date();
let age = today.getFullYear() - y;
if (today.getMonth() + 1 < m || (today.getMonth() + 1 === m && today.getDate() < day)) age--;

if (age < 18) return err(res, 'You must be at least 18 years old.', 400);
if (age > 85) return err(res, 'Please enter a valid date of birth.', 400);
