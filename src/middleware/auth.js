  ...
  try {
    ...
    if (decoded.role === 'rider') {
      const rider = await Rider
        ...
        .select('-password')
        .lean()
        .maxTimeMS(8000); // ← fail fast if DB is slow
    ...
    if (error.name === 'TokenExpiredError')  return err(res, 'Token expired. Please log in again.', 401);
    if (error.name === 'MongooseError' || error.code === 50) {
      // MaxTimeMSExpired — DB took too long
      return err(res, 'Server is slow. Please try again.', 503);
    }
    next(error);
  }
};
...
export const isAdmin = (req, res, next) => {
  if (req.role !== 'admin') return err(res, 'Access denied. Admins only.', 403);
  next();
};

export const isRider = (req, res, next) => {
  if (req.role !== 'rider') return err(res, 'Access denied. Riders only.', 403);
  next();
};

export const isCustomer = (req, res, next) => {
  if (req.role !== 'customer') return err(res, 'Access denied. Customers only.', 403);
  next();
};